...
class VideoUpload extends CI_Controller {
    ...
    public function index() {
        if ($this->input->post('video_upload')) {
          ...
          $upload_path = './upload/';
		// $upload_path = base_url('./upload/'.$config['']);
			
            $config['upload_path'] = $upload_path;
            //allowed file types. * means all types
            $config['allowed_types'] = 'wmv|mp4|avi|mov|gif|jpg|png';
            ...
            if (!$is_file_error) {
                ...
                if (!$this->upload->do_upload('video_name')) {
					...
					if($this->queries->insertImage($config)){
						echo 'path succesfully stored into the database';
